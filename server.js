...
function hasBingo(card, called) {
  const hit = (col, row) => { const val=card[col][row]; return val==='FREE'||called.includes(val); };
  for (let row=0;row<5;row++) { if ([0,1,2,3,4].every(col=>hit(col,row))) return true; }
  for (let col=0;col<5;col++) { if ([0,1,2,3,4].every(row=>hit(col,row))) return true; }
...
async function runGameCycle() {
  ...
  engine.running = true;
  try { await doLobby(); if (!engine.won) await doGame(); }
...
async function doLobby() {
  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━');
  engine.botCards={};engine.called=[];engine.won=false;
  ...
  await db.ref('game').set({started:false,paidOut:false,winner:null,pot:0,balls:null,ballCount:0,countdown:LOBBY_SECS,gameNum,phase:'lobby'});
  let pot=0;
  for (const b of BOTS) {
    const ref=db.ref(`users/${b.id}`),snap=await ref.once('value'),bal=snap.val()?.balance||50000;
    await ref.update({balance:Math.max(0,bal-ENTRY_FEE),gamesPlayed:(snap.val()?.gamesPlayed||0)+1});
    pot+=ENTRY_FEE;
    await db.ref(`lobby/players/${b.num}`).set({boardNum:b.num,telegramId:b.id,name:b.name,isBot:true,joinedAt:Date.now(),lastSeen:Date.now()});
    await db.ref(`lobby/taken/${b.num}`).set(b.num);
    engine.botCards[b.id]=mkCartela();
  }
  await db.ref('game/pot').set(pot);
  console.log(`🎰 Game #${gameNum} lobby open — 10 bots — pot: ${pot} ETB`);
  await new Promise(resolve=>{
    let secs=LOBBY_SECS;
    const tick=setInterval(async()=>{secs--;await db.ref('game/countdown').set(secs);if(secs%10===0)console.log(`⏱  ${secs}s`);if(secs<=0){clearInterval(tick);resolve();}},1000);
  });
}

async function doGame() {
  const potSnap=await db.ref('game/pot').once('value'),pot=potSnap.val()||0;
  await db.ref('game').update({started:true,startedAt:Date.now(),lastBallAt:Date.now()-1500,ballCount:0,pot,phase:'playing'});
  const playersSnap=await db.ref('lobby/players').once('value');
  console.log(`▶️  Game started — ${Object.keys(playersSnap.val()||{}).length} players — pot: ${pot} ETB`);
  const pool=Array.from({length:75},(_,i)=>i+1);
  engine.called=[];engine.won=false;
  await new Promise(resolve=>{
    engine.ballTimer=setInterval(async()=>{
      ...
      for(const[botId,card]of Object.entries(engine.botCards)){
        if(hasBingo(card,engine.called)){
          engine.won=true;clearInterval(engine.ballTimer);engine.ballTimer=null;
          await payBot(parseInt(botId),pot);resolve();return;
        }
      }
    },BALL_MS);
  });
...
async function payBot(botId,pot){
  const paidSnap=await db.ref('game/paidOut').once('value');if(paidSnap.val())return;
  const botCard=engine.botCards[botId];if(!botCard||!hasBingo(botCard,engine.called))return;
  const winnings=Math.floor(pot*WINNER_CUT),bot=BOTS.find(b=>b.id===botId);
  const ref=db.ref(`users/${botId}`),snap=await ref.once('value');
  await ref.update({balance:(snap.val()?.balance||0)+winnings,gamesWon:(snap.val()?.gamesWon||0)+1,totalWon:(snap.val()?.totalWon||0)+winnings});
  await db.ref('game').update({paidOut:true,winner:{boardNum:bot.num,telegramId:botId,name:bot.name,isBot:true,winnings,ts:Date.now(),card:botCard,calledNums:engine.called}});
...
bot.stopPolling()
  ...
  .then(async()=>{console.log('✅ Bot started. Admin:',ADMIN_ID);await ensureBots();runGameCycle();})
...
bot.onText(/\/start/, async msg => {
  ...
  const ref=db.ref(`users/${uid}`),sn=await ref.once('value');
  if(!sn.exists())await ref.set({telegramId:uid,name,username:msg.from.username||'',balance:0,inviteBonus:0,gamesPlayed:0,gamesWon:0,totalDeposited:0,totalWon:0,joinedAt:Date.now()});
  const startParam=msg.text?.split(' ')[1];
  if(startParam?.startsWith('ref_')&&!sn.exists()){
    const rid=startParam.replace('ref_','');
    if(rid!==String(uid)){const rref=db.ref(`users/${rid}`),rsnap=await rref.once('value');if(rsnap.exists()){await rref.update({inviteBonus:(rsnap.val().inviteBonus||0)+2});try{bot.sendMessage(parseInt(rid),'Someone joined with your invite! +2 ETB bonus');}catch(e){}}}
  }
  ...
  await bot.sendMessage(uid,
    ...
    {parse_mode:'Markdown',reply_markup:{inline_keyboard:[
      ...
      [{text:'🏧 Withdraw ETB',callback_data:'withdraw'},{text:'📋 Instructions',callback_data:'instructions'}],
      [{text:'🏆 Leaderboard',callback_data:'leaderboard'},{text:'👥 Invite +2 ETB',callback_data:'invite'}]
    ]}});
});
...
bot.on('callback_query', async query => {
  ...
  if(data==='withdraw'){
    const sn=await db.ref(`users/${uid}`).once('value'),u=sn.val();
    if(!u){bot.sendMessage(uid,'Please send /start first.');return;}
    if((u.balance||0)<50){bot.sendMessage(uid,`Minimum withdrawal is 50 ETB.\nYour balance: ${u.balance||0} ETB`);return;}
    bot.sendMessage(uid,
      `🏧 *Withdraw ETB*\n\nYour balance: *${u.balance} ETB*\nMinimum: *50 ETB*\n\nChoose which bank to receive your money:`,
      {parse_mode:'Markdown',reply_markup:{inline_keyboard:[
        [{text:'🏦 CBE Bank',callback_data:'wbank_cbe'}],
        [{text:'📱 Telebirr',callback_data:'wbank_telebirr'}],
        [{text:'🏛️ Bank of Abyssinia',callback_data:'wbank_abyssinia'}]
      ]}});
  }
  if(data.startsWith('wbank_')){
    const bk=data.replace('wbank_','');
    const sn=await db.ref(`users/${uid}`).once('value'),u=sn.val();
    if(!u)return;
    if((u.balance||0)<50){bot.sendMessage(uid,'Not enough balance.');return;}
    await db.ref(`pendingWithdraw/${uid}`).set({bankKey:bk,step:'awaiting_amount',ts:Date.now()});
    const bank=BANKS[bk];
    bot.sendMessage(uid,`${bank.emoji} *${bank.name}* selected\n\nYour balance: *${u.balance} ETB*\n\nHow much do you want to withdraw?\nMinimum: 50 ETB\n\nReply with the amount.\nExample: 100`,{parse_mode:'Markdown'});
  }
  if(data==='wd_player_confirm'){
    const wpSnap=await db.ref(`pendingWithdraw/${uid}`).once('value'),wp=wpSnap.val();
    if(!wp||wp.step!=='awaiting_confirm'){bot.sendMessage(uid,'No pending withdrawal found.');return;}
    const bank=BANKS[wp.bankKey];
    const user=(await db.ref(`users/${uid}`).once('value')).val();
    if((user?.balance||0)<wp.amount){bot.sendMessage(uid,'Your balance changed. Please start withdrawal again.');await db.ref(`pendingWithdraw/${uid}`).remove();return;}
    await db.ref(`users/${uid}`).update({balance:(user.balance||0)-wp.amount});
    const wid=`wd_${uid}_${Date.now()}`;
    await db.ref(`withdrawals/${wid}`).set({withdrawId:wid,userId:uid,userName:user?.name||'Player',bankKey:wp.bankKey,bankName:bank.name,amount:wp.amount,accountNumber:wp.accountNumber,accountName:wp.accountName,status:'pending',submittedAt:Date.now()});
    await db.ref(`pendingWithdraw/${uid}`).remove();
    bot.sendMessage(uid,`📤 *Withdrawal Submitted!*\n\nBank: *${bank.name}*\nAmount: *${wp.amount} ETB*\nAccount: *${wp.accountNumber}*\nName: *${wp.accountName}*\n\n⏳ Will be processed within 5-30 minutes.`,{parse_mode:'Markdown'});
    try{
      await bot.sendMessage(ADMIN_ID,
        `🏧 WITHDRAWAL REQUEST\n\nPlayer: ${user?.name||'Player'} (@${user?.username||'none'})\nBank: ${bank.name}\nAmount: ${wp.amount} ETB\nAccount No: ${wp.accountNumber}\nAccount Name: ${wp.accountName}\nUser ID: ${uid}`,
        {reply_markup:{inline_keyboard:[[
          {text:`✅ Confirm Sent ${wp.amount} ETB`,callback_data:`wd_confirm_${uid}_${wid}`},
          {text:'❌ Reject',callback_data:`wd_reject_${uid}_${wid}`}
        ]]}});
      console.log('Admin notified of withdrawal from',uid,'amount:',wp.amount);
    }catch(e){console.error('Admin withdraw notify failed:',e.message);}
  }
  if(data==='wd_player_cancel'){
    await db.ref(`pendingWithdraw/${uid}`).remove();
    bot.sendMessage(uid,'❌ Withdrawal cancelled. Your balance was not affected.');
  }
  if(data.startsWith('wd_confirm_')){
    if(uid!==ADMIN_ID)return;
    const parts=data.split('_'),tuid=parts[2],wid=parts[3];
    const wSnap=await db.ref(`withdrawals/${wid}`).once('value'),w=wSnap.val();
    if(!w||w.status!=='pending')return;
    await db.ref(`withdrawals/${wid}`).update({status:'completed',completedAt:Date.now()});
    const uref=db.ref(`users/${tuid}`),u=(await uref.once('value')).val();
    await uref.update({totalWithdrawn:(u?.totalWithdrawn||0)+w.amount});
    try{bot.editMessageReplyMarkup({inline_keyboard:[]},{chat_id:ADMIN_ID,message_id:query.message.message_id});}catch(e){}
    bot.sendMessage(ADMIN_ID,`✅ Withdrawal confirmed. ${w.amount} ETB sent to ${w.accountName}.`);
    bot.sendMessage(parseInt(tuid),`✅ *Withdrawal Completed!*\n\nAmount: *${w.amount} ETB*\nBank: *${w.bankName}*\nAccount: *${w.accountNumber}*\n\nMoney has been sent to your account! 🎉`,{parse_mode:'Markdown'});
    console.log(`Withdrawal confirmed: ${w.userName} withdrew ${w.amount} ETB`);
  }
  if(data.startsWith('wd_reject_')){
    ...
    if(!w)return;
    const uref=db.ref(`users/${tuid}`),u=(await uref.once('value')).val();
    if(u)await uref.update({balance:(u.balance||0)+w.amount});
    await db.ref(`withdrawals/${wid}`).update({status:'rejected',rejectedAt:Date.now()});
    try{bot.editMessageReplyMarkup({inline_keyboard:[]},{chat_id:ADMIN_ID,message_id:query.message.message_id});}catch(e){}
    bot.sendMessage(ADMIN_ID,'❌ Withdrawal rejected. Balance refunded to player.');
    bot.sendMessage(parseInt(tuid),`❌ *Withdrawal Rejected*\n\nYour ${w.amount} ETB has been returned to your balance.\nContact support if you believe this is an error.`,{parse_mode:'Markdown'});
  }
  if(data==='leaderboard'){
...
bot.on('message', async msg => {
  ...
  const wpsn=await db.ref(`pendingWithdraw/${uid}`).once('value'),wpend=wpsn.val();
  if(wpend){
    if(wpend.step==='awaiting_amount'){
      const amt=parseInt(text),u=(await db.ref(`users/${uid}`).once('value')).val();
      if(isNaN(amt)||amt<50){bot.sendMessage(uid,'Minimum withdrawal is 50 ETB. Enter a valid amount.');return;}
      if(amt>(u?.balance||0)){bot.sendMessage(uid,`You only have ${u?.balance||0} ETB. Enter a smaller amount.`);return;}
      await db.ref(`pendingWithdraw/${uid}`).update({step:'awaiting_account',amount:amt});
      bot.sendMessage(uid,`Amount: ${amt} ETB confirmed.\n\nNow enter your ${BANKS[wpend.bankKey].name} account number:`);
      return;
    }
    if(wpend.step==='awaiting_account'){
      await db.ref(`pendingWithdraw/${uid}`).update({step:'awaiting_name',accountNumber:text});
      bot.sendMessage(uid,`Account number saved.\n\nNow enter the full name on the account:`);
      return;
    }
    if(wpend.step==='awaiting_name'){
      ...
      if((user?.balance||0)<wpend.amount){
        bot.sendMessage(uid,'Your balance is not enough. Please start withdrawal again.');
        await db.ref(`pendingWithdraw/${uid}`).remove();return;
      }
      // Save name and show confirmation summary to player
      await db.ref(`pendingWithdraw/${uid}`).update({step:'awaiting_confirm',accountName:text});
      bot.sendMessage(uid,
        `📋 *Please confirm your withdrawal:*\n\n🏦 Bank: *${bank.name}*\n💵 Amount: *${wpend.amount} ETB*\n🔢 Account No: *${wpend.accountNumber}*\n👤 Account Name: *${text}*\n\nIs this correct?`,
        {parse_mode:'Markdown',reply_markup:{inline_keyboard:[[
          {text:'✅ Yes, Submit',callback_data:'wd_player_confirm'},
          {text:'❌ Cancel',callback_data:'wd_player_cancel'}
        ]]}});
      return;
    }
  }
  ...
  if(pend.step==='awaiting_txn'){
    ...
    await db.ref(`deposits/${did}`).set({depositId:did,userId:uid,userName:user?.name||msg.from.first_name,bankKey:pend.bankKey,bankName:bank.name,amount:pend.amount,txnRef:text,status:'pending',submittedAt:Date.now()});
...
app.post('/api/user', async(req,res)=>{
  ...
  if(!sn.exists())await ref.set({telegramId,name:name||'Player',username:username||'',balance:0,inviteBonus:0,gamesPlayed:0,gamesWon:0,totalDeposited:0,totalWon:0,joinedAt:Date.now()});
...
app.post('/api/payout', async(req,res)=>{
  ...
  await db.ref('game').update({paidOut:true,winner:{boardNum:null,telegramId,name:u.name,isBot:false,winnings:win,ts:Date.now()}});
  try{bot.sendMessage(parseInt(telegramId),`🎉 *BINGO! You Won!*\n\n🏆 Winnings: *${win} ETB*\n💳 New Balance: *${nb} ETB*\n\n🎊 Congratulations!`,{parse_mode:'Markdown',reply_markup:{inline_keyboard:[[{text:'🎯 Play Again',web_app:{url:MINI_APP_URL}}]]}});}catch(e){}
