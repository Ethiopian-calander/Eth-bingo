...
async function payBot(botId, pot) {
  ...
  await db.ref('game').update({
    paidOut: true, await db.ref('game/started').set(false);
      winner: {
      boardNum:   bot.num,
      telegramId: botId,
      name:       bot.name,
...
bot.on('callback_query', async query => {
  ...
  if (data==='withdraw') {
    ...
    if (!u){bot.sendMessage(uid,'Please send /start first.');return;}
    if ((u.balance||0) < 50){
      bot.sendMessage(uid,`Minimum withdrawal is 50 ETB.\nYour balance: ${u.balance||0} ETB`);return;
    }
    bot.sendMessage(uid,
      `🏧 Withdraw ETB\n\nYour balance: ${u.balance} ETB\nMinimum: 50 ETB\n\nChoose which bank to receive money:`,
  ...
  if (data.startsWith('wbank_')) {
    ...
    const bank=BANKS[bk];
    bot.sendMessage(uid,
      `${bank.emoji} ${bank.name} selected\n\nYour balance: ${u.balance} ETB\n\nHow much do you want to withdraw? (minimum 50 ETB)\n\nReply with the amount.\nExample: 100`);
  }
  if (data==='wd_player_confirm') {
    ...
    if ((user?.balance||0) < wp.amount){
      bot.sendMessage(uid,'Your balance changed. Please start withdrawal again.');
    ...
    await db.ref(`withdrawals/${wid}`).set({
      withdrawId:wid, userId:uid, userName:user?.name||'Player',
      bankKey:wp.bankKey, bankName:bank.name, amount:wp.amount,
      accountNumber:wp.accountNumber, accountName:wp.accountName,
      status:'pending', submittedAt:Date.now()
    });
    await db.ref(`pendingWithdraw/${uid}`).remove();
    bot.sendMessage(uid,
      `📤 *Withdrawal Submitted!*\n\nBank: *${bank.name}*\nAmount: *${wp.amount} ETB*\nAccount: *${wp.accountNumber}*\nName: *${wp.accountName}*\n\n⏳ Will be processed within 5-30 minutes.`,
      {parse_mode:'Markdown'});
    try {
  ...
  if (data.startsWith('wd_confirm_')) {
    ...
    const parts=data.split('_'), tuid=parts[2], wid=parts.slice(3).join('_');
    ...
    bot.sendMessage(parseInt(tuid),
      `✅ Withdrawal Completed!\n\nAmount: ${w.amount} ETB\nBank: ${w.bankName}\nAccount: ${w.accountNumber}\n\nMoney has been sent to your account!`);
